...
class _ExpenseScreenState extends State<ExpenseScreen> {
  ...
  Future<void> _showExpenseDialog({Expense? expense}) async {
    ...
    // Always use the raw yyyy-MM-dd value for the date field
    final dateController = TextEditingController(
      text: (expense != null)
          ? _extractRawDate(expense.date)
          : ''
    );
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // Always send only the first 10 characters (YYYY-MM-DD) to the backend
                ...
                rows: _expenses.map((expense) {
                  return DataRow(
                    cells: [
                      ...
                      DataCell(Text('\$' + NumberFormat('#,##0.00').format(expense.amount))),
                      DataCell(Text(
                        () {
                          try {
                            final inputFormat = DateFormat("EEE, d MMM yyyy HH:mm:ss 'GMT'");
                            final dateObj = inputFormat.parse(expense.date, true).toLocal();
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      '\$' + NumberFormat('#,##0.00').format(_expenses.fold(0.0, (sum, item) => sum + item.amount)),
